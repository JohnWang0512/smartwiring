...
- (void)drawRect:(CGRect)rect {
	...
	if (_annular) {
        ...
        CGFloat tmpAngle = endAngle - ((float)M_PI / 2); // 90 degrees
        if (tmpAngle < 0) {
            tmpAngle += ((float)M_PI * 2);
        }
        [processPath addArcWithCenter:center radius:radius startAngle:tmpAngle endAngle:endAngle clockwise:YES];
